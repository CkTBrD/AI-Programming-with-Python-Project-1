...
def calculates_results_stats(results_dic):
    ...
    #Set indexing conditions and all counter initial conditions that
    #will be used to process through images in results_dic
    ...
    for key in results_dic:
        ...
        # Pet Image  and Classifier Labels Match Exactly (idx 2)
        if results_dic[key][2] == 1: # pet image and classifier label match
            results_stats_dic['n_match'] += 1 #increment counter
            ...
            #           
            # Pet Image Label is a Dog AND Classifier Labels match- counts Correct Breed (idx 3)
            if results_dic[key][3] == 1: #pet image 'is-a' dog
                results_stats_dic['n_correct_breed'] += 1 #increment counter 


        # Pet Image Label is a Dog - counts number of dog images
        if results_dic[key][3] == 1: #pet image 'is-a' dog
            results_stats_dic['n_dogs_img'] += 1 #increment counter
            
            # Classifier classifies image as Dog (& pet image is a dog)
            # counts number of correct dog classifications
            if results_dic[key][4] == 1: #Classifier classifies image 'as-a' dog
                results_stats_dic['n_correct_dogs'] += 1 #increment counter 
            ...
            #           
            # Pet Image Label is NOT a Dog (idx 4). Classifier correctly classified image 
            # as not a dog. 
        else:
            
            # Classifier classifies image as NOT a Dog(& pet image isn't a dog)
            # counts number of correct NOT dog clasifications.
            if results_dic[key][4] == 0: #Not a dog
                results_stats_dic['n_correct_notdogs'] += 1 #increment counter 
    ...
    print(results_dic) # Print out put to check results. 
